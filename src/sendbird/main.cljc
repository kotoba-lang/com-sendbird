(ns sendbird.main
  "Kotodama WASM entrypoint for the Sendbird clean-room actor (L4) — Clojure port.
  ...
  facts (`sendbird.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sendbird")
(def tier "L4")
...
(def entity-specs
  [{:entity "User"         :plural "users"         :id-prefix "sendbird_use"
    :fields [:userId :nickname :profileUrl :accessToken :isActive :role :createdAt :lastSeenAt :isOnline]
    :required [:nickname :profileUrl]
    :coerce {:isActive :bool :isOnline :bool :lastSeenAt :int}
    :refs {:userId "User"}}
   {:entity "Message"      :plural "messages"      :id-prefix "sendbird_mes"
    :fields [:messageId :type :customType :message :channelUrl :createdAt :updatedAt :isRemoved]
    :required [:type :customType]
    :coerce {:messageId :int :isRemoved :bool}
    :refs {:messageId "Message"}}
   {:entity "GroupChannel" :plural "groupchannels" :id-prefix "sendbird_gro"
    :fields [:channelUrl :name :memberCount :isDistinct :createdAt]
    :required [:channelUrl :name]
    :coerce {:memberCount :int :isDistinct :bool}
    :refs {}}
   {:entity "OpenChannel"  :plural "openchannels"  :id-prefix "sendbird_ope"
    :fields [:channelUrl :name :participantCount :createdAt]
    :required [:channelUrl :name]
    :coerce {:participantCount :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"sendbird.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sendbird-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sendbird-compat/ready)
